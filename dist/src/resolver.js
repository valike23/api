...
const auth_1 = require("./helphers/auth");
exports.resolvers = {
    Query: {
        ...
        product: async (root, args, ctx, info) => {
            ...
            if (args.onlyData) {
                ...
                return {
                    ...
                    next: index < relatedProducts.length - 1
        ...
        products: async (root, args, ctx, info) => {
            ...
            for (let i = 0; i < sizeArray.length; i++) {
                if (sizeArray[i] == 'XS')
                    sizeArray[i] = 'Extra Small';
                else if (sizeArray[i] == 'S')
                    sizeArray[i] = 'Small';
                else if (sizeArray[i] == 'M')
                    sizeArray[i] = 'Medium';
                else if (sizeArray[i] == 'L')
                    sizeArray[i] = 'Large';
                else if (sizeArray[i] == 'XL')
                    sizeArray[i] = 'Extra Large';
            }
            let filteredProducts = products.filter((product) => {
                ...
                if (flag && (args.color.length || args.size.length)) {
                    flag = product.variants.find((variant) => (!args.color.length ||
                        (variant.color &&
                            args.color.find((item) => item == variant.color_name))) &&
                        (!args.size.length ||
                            (variant.size &&
                                variant.size.find((item) => sizeArray.find((sa) => sa == item.name)))));
                }
                if (flag) {
                    flag = false;
                    if (product.variants.length > 0) {
                        for (let i = 0; i < product.variants.length; i++) {
                            if (minPrice < product.variants[i].price &&
        ...
        login: (root, args, ctx, info) => {
            ...
            const authHelpher = new auth_1.Auth(email, password);
            return authHelpher.login();
        },
        register: (root, args, ctx, info) => {
            console.log('argument', args);
            const { password, email, name } = args;
            const authHelpher = new auth_1.Auth(email, password, name);
